<?php
require_once __DIR__ . "/../../Class/encabezado.php";
require_once __DIR__ . "/../../Controller/listarOrden.php";

$idDespacho = intval($_GET['idEncabezado']);
...
if ($idPrincipal !== $idDespacho) {
    // Obtener datos del principal para construir la URL correcta
    $despachoPrincipal = $encabezadoClass->obtenerDespachoPorId($idPrincipal);
    $urlPrincipal = 'editarOrden.php?idEncabezado=' . $idPrincipal;
    if ($despachoPrincipal) {
        $urlPrincipal .= '&proveedor=' . urlencode($despachoPrincipal['PROVEEDOR'])
            . '&ordenDeCompra=' . urlencode(trim($despachoPrincipal['ORDEN_COMPRA']))
            . '&codProveedor=' . urlencode($despachoPrincipal['COD_PROVEE'])
            . '&valorFobPeso=' . urlencode($despachoPrincipal['VALOR_FOB_PESO'] ?? 0);
    }
    ...
    Swal.fire({
        ...
        text: "Esta OC es hija de otra. Te redirigimos a la OC principal para editar los costos.",
        icon: "info",
        confirmButtonText: "Continuar",
        confirmButtonColor: "#7066e0",
        allowOutsideClick: false
    }).then(function() {
        window.location.href = "' . addslashes($urlPrincipal) . '";
    });
    </script></body></html>';
    exit;
}

$ordenCompra = $idDespacho;
